...
task :link_shared do
  ...
  unless remote_file_exists?("#{shared_path}/old/settings.yaml")
    puts " creating settings file ".center(80, '-')
    run "mkdir -p #{shared_path}/old"
    run "cp -r #{release_path}/old/shared/* #{shared_path}/old"
  end
  run "rm -f #{release_path}/db/#{db_file}"
  run "ln -s #{shared_path}/db/#{db_file} #{release_path}/db/#{db_file}"
  run "mv #{release_path}/old/shared #{release_path}/old/shared_save"
  run "ln -s #{shared_path}/old #{release_path}/old/shared"
